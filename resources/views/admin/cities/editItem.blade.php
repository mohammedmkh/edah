            ...
            <div class="row">
                    <div class="col-xl-12 order-xl-1">
                        <div class="card form-card bg-secondary shadow">
                            ...
                            <div class="card-body">
                                <form method="post" action="{{url(adminPath().'cities/'.$data->id)}}" autocomplete="off" enctype="multipart/form-data" >
                                    @csrf
                                    @method('put')
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-6">
                                                <div class="form-group{{ $errors->has('shop_id') ? ' has-danger' : '' }}">
                                                    ...
                                                    <select name="country_id" id="input-country_id" class="form-control form-control-alternative{{ $errors->has('country_id') ? ' is-invalid' : '' }}"required>
                                                        <option value="">الدولة</option>
                                                        @foreach ($countries as $item)
                                                            <option value="{{$item->id}}" {{ $data->country_code==$item->code ? 'Selected' : ''}}>{{$item->name}}</option>
                                                        @endforeach
                                                    </select>
